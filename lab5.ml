...
let is0_255 (c : int * int * int) : bool =
  let (r, g, b) = c in
  r >= 0 && r <= 255
  && g >= 0 && g <= 255
  && b >= 0 && b <= 255 ;;
...
let validated_rgb (c : color) : color = 
  match c with
  | Simple col -> Simple col
  | RGB(r, g, b)  -> if is0_255 (r, g, b) then RGB(r, g, b)
                      else raise (Invalid_color "invalid RGB value");;
...
let make_color (r : int) (g : int) (b : int) : color = 
  let col = RGB(r, g, b) in validated_rgb col ;;
...
let validated_date (d : date) : date = 
  let is_leap (y : int) : bool = 
    if not (y mod 4 = 0) then false
    else if (y mod 4 = 0) && (y mod 400 = 0) then true
    else false
in
  let in_range (start_day : int) (last_day : int) (some_day) : bool =
    start_day <= some_day && some_day <= last_day in
  let dies = d.day in
  let mon =  d.month in
  let c = d.year in
  match mon with
  | 2 -> if (in_range 0 28 dies) then {day =  dies; month = mon; year = c}
         else if (dies = 29) && is_leap c then {day =  dies; month = mon; year = c}
         else raise (Invalid_date "invalid day or year")
  | 1 | 3 | 5 | 7 | 8 | 10  | 12 ->
      if (in_range 0 31 dies) then {day =  dies; month = mon; year = c}
      else raise (Invalid_date "invalid day")
  | 4 | 6 | 9  | 11   ->
      if (in_range 0 30 dies) then {day =  dies; month = mon; year = c}
      else raise (Invalid_date "invalid day")
...
let marry (f : family) (p : person) : family = 
  match f with
  | Single babe -> Family(babe, p, [])
  | _ -> raise (Family_Trouble "this person already has a family") ;;
